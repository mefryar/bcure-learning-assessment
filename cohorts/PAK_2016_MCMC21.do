...
*		training held at three of the National Institutes of Management
*		which are the constituent units of the National School of 
*		Public Policy in Pakistan for officers in the 21st Mid-Career
*		Management Cohort. The trainings took place from April 21-23 at 
*		NIM Karachi, April 26-28 at NIM Lahore, and May 18-20 at NIM 
...
* Uses: 	PAK_2016_MCMC21_Karachi_Pre.csv,
*		PAK_2016_MCMC21_Lahore_Pre.csv,
*		PAK_2016_MCMC21_Quetta_Pre.csv, 
*		PAK_2015_MCMC21_Karachi_Post.csv,
*		PAK_2016_MCMC21_Lahore_Post.csv, 
*		PAK_2016_MCMC21_Quetta_Post.csv 
* Creates: 	PAK_2016_MCMC21.dta
...
forval i = 1/2 {
	...
	/* Since training was administered on three campuses, we need to clean 
	six files */
	forv j = 1/3 {
		local campus: word `j' of "karachi" "lahore" "quetta"
...
if r(N) != 0 {
	...
	must be examined and regrouped manually */
		...
		assert cohort[1] == "pak_2016_mcmc21"
		
		// Manually ungroup those with strgroup incorrectly grouped 
		replace group = 41 if fullname == "Muhammad Kaleem"

		/* Manually group those which strgroup failed to group
		correctly */

		forv i = 1/8 {
			local pre_group: word `i' of   2  3  4  8 14 15 20 25
			local post_group: word `i' of 33 30 31 29 35 36 37 40
