...
*		training held at the five National Institutes of Management
*		which are the constituent units of the National School of 
*		Public Policy in Pakistan for officers in the 22nd Mid-Career
*		Management Cohort. The trainings took place from Sept. 5-9 at
*		NIM Lahore, Sept. 19-23 at NIM Islamabad, Sept. 26-30 at NIM 
*		Peshawar, Oct. 14-17 at NIM Karachi and Oct. 17-21 at NIM 
*		Quetta. Each training covered the BCURE modules: Systematic 
*		Approaches to Policy Decisions (SYS), Becoming an Effective 
*		Consumer of Cost-Benefit Analysis (CBA), Becoming an Effective 
*		Consumer of Descriptive Evidence (DES), Becoming an Effective 
*		Consumer of Impact Evaluations (IMP), Aggregating Evidence 
*		(AGG) and Commissioning Evidence (COM).
* Uses: 	PAK_2016_MCMC22_Islamabad_Pre.csv,
*		PAK_2016_MCMC22_Karachi_Pre.csv,
*		PAK_2016_MCMC22_Lahore_Pre.csv,
*		PAK_2016_MCMC22_Quetta_Pre.csv, 
*		PAK_2016_MCMC22_Peshawar_Pre.csv, 
*		PAK_2015_MCMC22_Islamabad_Post.csv,
*		PAK_2015_MCMC22_Karachi_Post.csv,
*		PAK_2016_MCMC22_Lahore_Post.csv, 
*		PAK_2016_MCMC22_Quetta_Post.csv 
*		PAK_2016_MCMC22_Peshawar_Post.csv 
* Creates: 	PAK_2016_MCMC22.dta
...
forval i = 1/2 {
	...
	/* Since training was administered on five campuses, we need to clean 
	ten files */
	forval j = 1/5 {
		local campus: word `j' of "islamabad" "karachi" "lahore" ///
					  "quetta"    "peshawar"
		...
		/* There were two different officers Muhammad Naeem so we must 
		distinguish them based on campus so that fullname can be used
		as a key for merging */
		replace fullname = fullname + " (`campus')" ///
			if fullname == "Muhammad Naeem"
...
if r(N) != 0 {
	...
	must be examined and regrouped manually */
		...
		assert cohort[1] == "pak_2016_mcmc22"
		
		// Manually ungroup those with strgroup incorrectly grouped 
		replace group = 65 if fullname == "Iqbalahmedshaikh" ///
					| fullname == "Iqbal Ahmed Shaikh"
		replace group = 66 if fullname == "Muhammad.Arif" ///
					| fullname == "Muhammad Arif"
		replace group = 67 if fullname == "Muhammad Farooq"
		replace group = 68 if fullname == "Muhammad Naeen"
		replace group = 69 if fullname == "Muhammad Masood"
		replace group = 70 if strpos(fullname,"Sajjad Hussain Shah")

		/* Manually group those which strgroup failed to group
		correctly */
		forv i = 1/10 {
			local pre_group: word `i' of   3  4 16 25 30 31 33 ///
				39 46 56
			local post_group: word `i' of 47 49 55 57 58 60 62 ///
				52 64 68
