...
*		training held at the National School of Public Policy in 
*		Lahore, Pakistan for officers in the 105th National Management
*		Cohort. The trainings took place from December 5-6 and covered 
*		the BCURE modules: Systematic Approaches to Policy Decisions 
*		(SYS), and Becoming an Effective Consumer of Impact Evaluations 
*		(IMP).
* Uses: 	PAK_2016_NMC105_Pre.csv, PAK_2016_NMC105_Post.csv 
* Creates: 	PAK_2016_NMC105.dta, PAK_2016_NMC105_score.xlsx
...
if r(N) != 0 {
	...
	must be examined and regrouped manually */
		...
		assert cohort[1] == "pak_2016_nmc105"
		
		assert group_matched <= 1 if !missing(group)

		sort group _merge
		list fullname group *motivation if group_matched == 1

		sort fullname
		list fullname group *motivation if group_matched == 0
...
// Create local containing cohort identifiers
...
foreach cohort in `cohorts'{
	sum post_total_score if cohort=="`cohort'"
	replace adjustment = `total_questions' - r(max) if cohort=="`cohort'"
}
...
// Keep only necessary variables
keep cohort fullname *total_score* post_*_score 
order cohort fullname adjusted_post_total_score_pct ///
