...
*		training held from December 22 to 23, 2017 for the IAS Officers 
*		participating in the Phase I Professional Course at the 
*		Lal Bahadur Shastri National Academy of Administration covering 
*		the BCURE module on Becoming an Effective Consumer of 
*		Descriptive Evidence (DES). 
* Uses: 	IND_2016_PhaseI_Pre.csv, IND_2016_PhaseI_Post.csv
* Creates: 	IND_2016_PhaseI.dta
...
forval i = 1/2 {
	...
	display "Cleaning `cohort' `period'"

	cd "$bcure_learning_assessment/data/raw/`period'"
	import delimited `cohort'_`period'.csv, ///
		varnames(1) clear
	...
	// Save data from pre-test in temporary file
	if "`period'" == "pre" {
...
if r(N) != 0 {
	...
	must be examined and regrouped manually */
		...
		assert cohort[1] == "ind_2016_phasei"

		drop if inlist(fullname,"H","S","Neeraj Trivedi")

		forv i = 1/24 {
			local pre_group: word `i' of 99 100 101 102 105 107 ///
				108 109 110 111 112 113 114 116 117 120 121 ///
				122 123 124 128 118 129 132
			local post_group: word `i' of 2  3   4   6   11  19 ///
				21  23  25  26  32  38  40  45  49  61  62  ///
				66  74  75  88  92  95  98
			quietly replace group =  `post_group' ///
				     if group == `pre_group'
		}
		forv i = 1/3 {
			local misgrouped_names: word `i' of "Anandh K" ///
				"Anand O" "Vishal Singh"
			local new_group: word `i' of 104 103 130
			quietly replace group = `new_group' ///
				if fullname == "`misgrouped_names'"
		}
