...
*		training held in two parts, first, from April 8-9, 2015 
*		and, second, from July 6-7, 2015 for the civil service
*		officers of the 17th Senior Career Management Cohort at the
*		National School of Public Policy in Karachi, Pakistan covering 
...
* Uses: 	PAK_2015_SMC17_Pre.csv, PAK_2015_SMC17_Post.csv
* Creates: 	PAK_2015_SMC17.dta
...
forval i = 1/2 {
	...
	forv j = 1/2 {
		...
		/* Note: Policy vignette questions were administered during 
		April pre-test so questions need to be renumbered */
		if "`period'" == "pre" & "`units'" == "cba_des_sys"{
			drop q3-q61
			rename q62_1-q65 pre_sys#, addnumber
			rename q66_1-q69 pre_cba#, addnumber
			rename q70_1-q73 pre_des#, addnumber
			rename (q78 q79) (q18 q19)
		}
		...
		/* Save data from the July AGG, COM, IMP session in 
		temporary file */
		if "`units'" == "agg_com_imp" {
			tempfile `cohort'_`period'_july 	
			save ``cohort'_`period'_july'
		}

		else if "`units'" == "cba_des_sys" {
			merge 1:1 fullname ///
				using ``cohort'_`period'_july'
			...
			if r(N) == 1 drop if group_matched == 0
				...
				if "`period'" == "pre" {
					forv i = 1/15 {
						/* Manually group obs. which 
						strgroup failed to group 
						properly */
						loc pre_group: word `i' of ///
							 1  3  6  9 ///
							10 11 13 15 17 18 ///
							20 21 22 23 24
						loc post_group: word `i' of ///
							28 38 27 29 ///
							30 32 35 31 34 36 ///
							37 26 33 39 41
						replace group=`post_group' ///
							if group==`pre_group'
					}
				}
			...
			CBA, DES and SYS with matching data from the session
			that covered AGG, COM and IMP. Since the data from the
			session that covered CBA, DES and SYS was the "master" 
			during the merge, the data is sorted by group and then 
			_merge so that the first observation within each group 
			is from the session covering CBA, DES and SYS. */
			sort group _merge
			foreach score of varlist *agg* *com* *imp* {
				by group: replace `score' = `score'[_n+1] ///
					if _merge == 1 
			}
			
			// Drop AGG, COM and IMP observations
			drop if _merge == 2 
...
if r(N) != 0 {
	...
	must be examined and regrouped manually */
		...
		assert cohort[1] == "pak_2015_smc17"
		
		forv i = 1/3 {
			/* Manually group obs. which strgroup failed to group 
			properly */
			loc pre_group: word `i' of   2  3  5
			loc post_group: word `i' of  7 11 12
		...
		list fullname group pre*motivation post*motivation ///
			if group_matched_new == 1 & !missing(group)
	...
	/* Check (We do not want observations that have only April or only July
