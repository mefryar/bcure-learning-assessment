...
*		Lahore, Pakistan which completed blended learning sessions
*		covering five of the BCURE modules: Systematic Approaches to
*		Policy Decisions (SYS), Becoming an Effective Consumer of 
*		Cost-Benefit Analysis (CBA), Becoming an Effective Consumer of 
*		Impact Evaluations (IMP), Aggregating Evidence (AGG), and
*		Commissioning Evidence (COM). The cohort was divided into four
*		academic groups which completed the digital units and
*		accompanying in-class sessions in four day sessions that took
*		place between January 18 and February 8, 2017. This cohort had
*		previously completed "online-only" versions of the other two
*		BCURE modules, Becoming an Effective Consumer of Descriptive
*		Evidence (DES) and Becoming an Effective Consumer of Impact
*		Evaluations (IMP), between October 26 and November 9, 2016.
* Uses: 	pak_2017_ctp44_blended_pre.csv, pak_2017_ctp44_blended_post.csv
* Creates: 	pak_2017_ctp44.dta
...
forval i = 1/2 {
	...
	import delimited `cohort'_blended_`period'.csv, varnames(1) clear
	...
	// For CTP 44, there were 4 academic groups
	rename q41 academic_group

	// Keep only necessary variables
	keep cohort academic_group `period'_* fullname female
...
if r(N) != 0 {
	...
	must be examined and regrouped manually */
		...
		assert cohort[1] == "pak_2017_ctp44"

		/* Manually group those which strgroup failed to group
		correctly */
		forv i = 1/5 {
			local pre_group: word `i' of  26 36 37 39 42
			local post_group: word `i' of 4  8  13 14 19
