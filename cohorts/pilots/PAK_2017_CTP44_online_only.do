...
* Description: 	Cleans learning assessment data from the 44th CTP cohort in 
*		Lahore, Pakistan which completed "online-only" versions of 
*		Becoming an Effective Consumer of Descriptive Evidence (DES) 
*		and Becoming an Effective Consumer of Impact Evaluations (IMP), 
*		between October 26 and November 9, 2016.
* Uses: 	pak_2016_ctp44_online_des_pre.csv
*		pak_2016_ctp44_online_imp_pre.csv
*		pak_2016_ctp44_online_des_post.csv
*		pak_2016_ctp44_online_imp_post.csv
* Creates: 	pak_2016_ctp44_online.dta
...
local cohort "pak_2016_ctp44_online"
...
forval i = 1/2 {
	...
	cd "$bcure_learning_assessment/data/raw/pilots/`period'"

	/* Since the pre-post tests were embedded in each unit, we need to  
	clean four files */
	forval j = 1/2 {
		local unit: word `j' of "des" "imp"

		import delimited `cohort'_`unit'_`period'.csv, ///
			varnames(1) clear
		...
		/* Note that numbering of motivation and incentive questions is 
		different and that the questions were asked in each test */
		rename  (q27 q28) ///
			(`period'_`unit'_motivation `period'_`unit'_incentives)
		...
		keep cohort unit `period'_* fullname female

		// Identify cohort
		replace cohort = "`cohort'"
		
		// Save data from descriptive unit in temporary file
		if "`unit'" == "des" {
			tempfile `cohort'_`period'_`unit' 	
			save ``cohort'_`period'_`unit''
		}

		// Merge data from impact unit with data from descriptive unit
		else if "`unit'" == "imp" {
			merge 1:1 fullname using ``cohort'_`period'_des' 
			...
			/* Otherwise, observations which strgroup failed to 
			group correctly must be regrouped manually for DES and
			IMP tests*/
			if `i' == 1{
				/* Manually ungroup those which strgroup 
				incorrectly grouped */
				replace group = 78 if fulln == "Muhammad Qasim"
				replace group = 79 if fulln == "Muhammad Nabil"
				replace group = 80 if fulln == "Muhammad Azhar"
				replace group = 81 if fulln == "Muhammad Usman"

				/* Manually group those which strgroup failed 
				to group correctly */
				forv i = 1/5 {
				 	local des_group: ///
				 		word `i' of  7 15 20 30 35
				 	local imp_group: ///
				 		word `i' of 41 53 58 68 77

				 	replace    group =  `imp_group' ///
				 		if group == `des_group'
				}

				/* Manually ungroup those which strgroup
				incorrectly grouped */
				replace group = 82 if fullname == "Rabia"
				replace group = 22 if strpos(fullname,"Rabbia")
			}
				...
				forv i = 1/6 {
				 	local des_group: ///
				 		word `i' of  3  6 26 29 31 33
				 	local imp_group: ///
				 		word `i' of 35 37 48 53 57 60
				...
				/* Manually ungroup those which strgroup
				incorrectly grouped */
				replace group = 61 if strpos(fullname,"Shoaib")
 			}
			...
			/* Replace missing data with matching data. Since IMP 
			data was the "master" during the merge, the data is 
			sorted by group and then _merge so that the first 
			observation within each group is the IMP data. */
			sort group _merge
			foreach score of varlist *des* {
				by group: replace `score' = `score'[_n+1] ///
					if _merge == 1 
			}
			
			// Drop DES observations
			drop if _merge == 2 
...
count if !mi(pre_des_motivation) & !mi(pre_imp_motivation) ///
	& !mi(post_des_motivation) & !mi(post_imp_motivation)
...
if r(N) != 0 {
	strgroup fullname if mi(pre_des_incentiv) | mi(pre_imp_incentiv) ///
	| mi(post_des_motivation) | mi(post_imp_motivation), ///
				gen(group) threshold(0.3) normalize(longer)
	...
	must be examined and regrouped manually */
		...
		replace group = 78 if fullname == "Muhammad Chanzaib"
		replace group = 79 if fullname == "Muhammad Shoaib"
		replace group = 80 if fullname == "Shahrukh A Khan"

		/* Manually group those which strgroup failed to group
		correctly */
		forv i = 1/6 {
			local pre_group: word `i' of  37 40 57 65 69 72
			local post_group: word `i' of  2  5 20 25 27 28
			replace group = `post_group' if group == `pre_group'
		}

		// Note Tehreem Bano and Tehreem Ilyas are distinct officers

		duplicates tag group, generate(group_matched_new)
	...
	/* Check (We do not want observations that have only DES or only IMP
...
order cohort-post_imp_incentives, alphabetic
...
save "$bcure_learning_assessment/data/coded/pilots/`cohort'.dta", replace
